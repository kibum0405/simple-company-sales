<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-text>
            <String v-if="editMode" label="SalesOrderNumber" v-model="value.salesOrderNumber" :editMode="editMode" :inputUI="''"/>
	...
	async created() {
        ...
        methods: {
            ...
            async createSalesOrder(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['createsalesorder'].href), params)
            ...
            openCreateSalesOrder() {
                this.createSalesOrderDiagram = true;
            },
            closeCreateSalesOrder() {
                this.createSalesOrderDiagram = false;
            },
